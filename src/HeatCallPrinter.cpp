...
public:
    ...
    HeatCallGraphInfo(Module *M, CallGraph *CG, function_ref<BlockFrequencyInfo *(Function &)> LookupBFI){
        ...
        for (Function &F : *M) {
            ...
            if (UseCallCounter) {
                std::optional<Function::ProfileCount> freq = F.getEntryCount();
                if (freq.has_value())
                localMaxFreq = freq.value().getCount();
            }
            else {
    ...
    void removeParallelEdges() {
        for (auto &I : (*CG)) {
            ...
            while (foundParallelEdge) {
                ...
                for (std::vector<CallGraphNode::CallRecord>::iterator CI = Node->begin(); CI!=Node->end(); CI++) {
...
template <>
struct GraphTraits<HeatCallGraphInfo *> : public GraphTraits<const CallGraphNode *> {
...
struct DOTGraphTraits<HeatCallGraphInfo *> : public DefaultDOTGraphTraits {
   ...
   static std::string getGraphName(HeatCallGraphInfo *Graph) {
      return "Call graph of module " + std::string(Graph->getModule()->getModuleIdentifier());
   }

   static bool isNodeHidden(const CallGraphNode *Node, const HeatCallGraphInfo *info) {
      if (FullCallGraph)
         return false;
    ...
    // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
    typedef mapped_iterator<CallGraphNode::const_iterator, decltype(&CGGetValuePtr)> nodes_iterator;
...
PreservedAnalyses HeatCallGraphDOTPrinterPass::run(Module &M, ModuleAnalysisManager &AM) {
    // 执行 pass 逻辑
    ...
    errs() << "\n";
    // 返回 PreservedAnalyses，表示该 pass 会保留所有分析结果
