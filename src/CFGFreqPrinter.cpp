...
static cl::opt<bool>
HeatCFGPerFunction("freq-cfg-per-function", cl::init(false), cl::Hidden,
...
struct DOTGraphTraits<HeatCFGInfo *> : public DefaultDOTGraphTraits {
    ...
    static std::string getGraphProperties(HeatCFGInfo *) {
        std::string GraphProps = "graph [splines=true overlap=false];\n";
        std::string NodeProps = "node [shape=box, style=\"rounded,filled\", color=black, fillcolor=gray40, fontsize=16, fontcolor=white, penwidth=2];";
        return GraphProps + NodeProps;
    }

    static std::string getGraphName(HeatCFGInfo *heatCFG) {
        return "CFG Frequency for function '" + heatCFG->getF()->getName().str() + "'";
    }
    ...
    static std::string getSimpleNodeLabel(const BasicBlock* Node,
        ...
        return std::to_string(Node->getNumber()) + ": " + (hci->getFreq(Node) == 0 ? "0" : std::to_string(hci->getFreq(Node)));
    }
    ...
    std::string getEdgeAttributes(const BasicBlock *Node, const_succ_iterator I,
        ...
        const Instruction* TI = Node->getTerminator(); // TerminatorInst has been removed
...
static void writeHeatCFGToDotFile(Function &F, BlockFrequencyInfo *BFI,
  ...
  std::string Filename = ("cfgreq." + F.getName() + ".dot").str();
...
PreservedAnalyses CFGFreqPrinterPass::run(Module &M, ModuleAnalysisManager &AM) {
    auto LookupBFI = [&](Function &F) {
        return &AM.getResult<FunctionAnalysisManagerModuleProxy>(M).getManager().getResult<BlockFrequencyAnalysis>(F);
    };
