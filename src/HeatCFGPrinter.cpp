...
PreservedAnalyses HeatCFGPrinterPass::run(Module &M, ModuleAnalysisManager &AM) {
...
PreservedAnalyses HeatCFGOnlyPrinterPass::run(Module &M, ModuleAnalysisManager &AM) {
    auto LookupBFI = [&](Function &F) {
        return &AM.getResult<FunctionAnalysisManagerModuleProxy>(M).getManager().getResult<BlockFrequencyAnalysis>(F);
    };
